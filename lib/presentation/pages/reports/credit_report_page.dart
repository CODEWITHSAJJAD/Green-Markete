                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              icon: const Icon(MingCuteIcons.mgc_share_2_line, size: 20),
  ...
  Future<void> _shareReport(String businessId) async {
    ...
    if (rows.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No credit data to share')),
      );
      return;
    }
      ...
      final bill = BillModel(
        ...
        sections: [
          BillSection('Outstanding Balances', [
            for (final r in rows)
              BillLine(r.fullName, CurrencyFormatter.format(r.outstandingBalance)),
          ]),
        ],
        total: BillLine(
          'Total Outstanding',
  ...
  Future<void> _shareCustomerStatement(CreditReportModel c) async {
    ...
    final bill = BillModel(
      ...
      header: [
        ...
        if (c.phone != null && c.phone!.isNotEmpty) BillHeaderLine('Phone', c.phone!),
        if (c.city != null && c.city!.isNotEmpty) BillHeaderLine('City', c.city!),
        BillHeaderLine('Date', DateFormatter.toDDMMYYYY(DateTime.now())),
      ],
      sections: [
        BillSection('Account Summary', [
          BillLine('Amount due on credit sales', CurrencyFormatter.format(c.outstandingBalance)),
        ]),
      ],
      total: BillLine('Outstanding Balance', CurrencyFormatter.format(c.outstandingBalance), emphasize: true),
