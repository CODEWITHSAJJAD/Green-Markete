...
class _PLReportPageState extends State<PLReportPage> {
  ...
  Future<void> _shareReport(String businessId) async {
    ...
    final bill = BillModel(
      ...
      header: [
        if (_from != null) BillHeaderLine('From', DateFormatter.toDDMMYYYY(_from!)),
        if (_to != null) BillHeaderLine('To', DateFormatter.toDDMMYYYY(_to!)),
        BillHeaderLine('Batches Included', '${pl.totalBatches}'),
      ],
      sections: [
        BillSection('Summary', [
          BillLine('Total Cost', CurrencyFormatter.format(pl.totalCost)),
          BillLine('Total Revenue', CurrencyFormatter.format(pl.totalRevenue)),
        ]),
        BillSection('Per Batch', [
          for (final b in pl.batchSummaries)
            BillLine(
              b.batchCode ?? 'Batch',
              CurrencyFormatter.format(b.netProfitLoss),
            ),
        ]),
      ],
      total: BillLine('Net Profit / Loss', CurrencyFormatter.format(pl.totalProfitLoss), emphasize: true),
      footer: 'Generated by MandiRoznamcha on ${DateFormatter.toDDMMYYYY(DateTime.now())}. '
    ...
    await shareBill(
      ...
      bill: bill,
      fileName: 'pl_report',
      subject: 'MandiRoznamcha — P&L Report',
    );
  }
